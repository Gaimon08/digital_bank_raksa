...
class Kredit_pegawai extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('Kredit_pegawai_m');
        $this->load->library('form_validation');
    }

    public function index()
	{
		$data['row'] = $this->Kredit_pegawai_m->get();
		$this->template->load('template' , 'kredit/kredit_pegawai/data_kredit_pegawai' , $data);
	}

    // Menampilkan form pengajuan kredit
    public function tambah() {
        $this->template->load('template' , 'kredit/kredit_pegawai/kredit_pegawai_add');
    }
    ...
    public function simpan() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $result = $this->Kredit_pegawai_m->simpan_pengajuan_kredit($data);
            ...
            if ($result) {
                ...
                redirect('kredit/kredit_pegawai');
            } else {
...
public function edit($id) {
    $data['pengajuan_kredit'] = $this->Kredit_pegawai_m->get_pengajuan_kredit_by_id($id);
    $this->template->load('template' , 'kredit/kredit_pegawai/kredit_pegawai_edit',$data);
}

// Menyimpan perubahan data pengajuan kredit
public function update() {
    $id = $this->input->post('id_kredit_pegawai'); // Ambil ID pengajuan kredit dari form
    ...
    if ($this->form_validation->run() == FALSE) {
        ...
        $data['pengajuan_kredit'] = $this->Kredit_pegawai_m->get_pengajuan_kredit_by_id($id);
        $this->template->load('template', 'kredit/kredit_pegawai/kerdit_pegawai_edit', $data);
    } else {
        ...
        $result = $this->Kredit_pegawai_m->update_pengajuan_kredit($id, $data);
     ...
     public function hapus($id) {
        $result = $this->Kredit_pegawai_m->hapus_pengajuan_kredit($id);
